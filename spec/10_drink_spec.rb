...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_choice) {described_class.new('coffee', 16)}
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_choice.type).to eq('coffee')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_choice.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:mydrink) { described_class.new}
      # remove the 'x' before running this test
      it 'is full' do
        expect(mydrink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:myjuice) { described_class.new('juice', 10)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(myjuice).not_to be_full
      end
    end
  end
end
