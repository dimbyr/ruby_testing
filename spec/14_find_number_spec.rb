...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_five_nine) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        guess = game_five_nine.make_guess
        expect(guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_eight_nine) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        guess = game_eight_nine.make_guess
        expect(guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_zero_three) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        guess = game_zero_three.make_guess
        expect(guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_three_three) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        guess = game_three_three.make_guess
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:ending_guess) {double('ending_guess', value: 6)}
      subject(:game_tobe_over) { described_class.new(6, 7, ending_guess, 6) }
      it 'is game over' do
        expect(game_tobe_over).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:continue_guess) { double('continue_guess', value: 6) }
      subject(:game_not_over) { described_class.new(6, 8, continue_guess, 7) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:game_five_eight) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        game_five_eight.update_range
        min = game_five_eight.min
        max = game_five_eight.max
        expect(min).to eq(max)
      end

      it 'does not update max' do
        game_five_eight.update_range
        expect(game_five_eight.max).to eq(8)
      end
    end
  end
end
